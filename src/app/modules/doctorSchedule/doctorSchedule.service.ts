...
const getAllDoctorSchedulesFromDB = async (options: IOptions, filters: any) => {
  ...
  const whereConditions = buildWhereCondition<Prisma.PatientWhereInput>([], filters);
...
const getMyScheduleFromDB = async (filters: any, options: IPaginationOptions, user: JwtPayload) => {
  const { page, limit, skip, sortBy, orderBy } = paginationHelper.calcPagination(options);
...
const deleteDoctorScheduleByIdFromDB = async (user: JwtPayload, scheduleId: string) => {
  const doctor = await prisma.doctor.findFirstOrThrow({
    where: {
      email: user.email,
    },
