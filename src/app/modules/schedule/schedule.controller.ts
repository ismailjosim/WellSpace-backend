...
const getScheduleForDoctor = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await scheduleService.getScheduleForDoctorFromDB(user, filters, options);
  sendResponse(res, {
...
const getScheduleById = catchAsync(async (req: Request, res: Response) => {
  const result = await scheduleService.getScheduleByIdFromDB(req.params.id as string);
  sendResponse(res, {
    statusCode: StatusCode.OK,
    success: true,
...
const deleteDateRangeSchedule = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!startDateTime || !endDateTime) {
    throw new AppError(StatusCode.BAD_REQUEST, 'Both startDateTime and endDateTime are required');
  }
  ...
  if (isNaN(start.getTime()) || isNaN(end.getTime())) {
    throw new AppError(StatusCode.BAD_REQUEST, 'Invalid date format. Use ISO 8601 format');
  }
  ...
  const result = await scheduleService.deleteDateRangeScheduleFromDB(startDateTime, endDateTime);
