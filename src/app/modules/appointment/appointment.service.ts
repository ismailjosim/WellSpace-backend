...
import { AppointmentStatus, PaymentStatus, UserRole, type Prisma } from '@prisma/client';
...
const createAppointmentIntoDB = async (
  ...
  await NotificationService.createAppointmentBookedNotifications(result.appointmentId);
...
const getAllAppointmentFromDB = async (options: IOptions, filters: any) => {
  ...
  const whereConditions = buildWhereCondition<Prisma.AppointmentWhereInput>(undefined, filters);
...
const getMyAppointmentFromDB = async (options: IOptions, filters: any, user: JwtPayload) => {
  const { page, limit, skip, sortBy, orderBy } = paginationHelper.calcPagination(options);
  const whereConditions = buildWhereCondition<Prisma.AppointmentWhereInput>(undefined, filters);
...
const updateAppointmentStatusInfoDB = async (
  ...
  if (user.role === UserRole.DOCTOR) {
    ...
    if (user.email !== appointmentData.doctor.email) {
      throw new AppError(StatusCode.FORBIDDEN, 'You are not authorized to update this appointment');
    }
...
const initiatePaymentForAppointment = async (appointmentId: string, user: JwtPayload) => {
  ...
  if (appointment.status === AppointmentStatus.CANCELED) {
    throw new AppError(StatusCode.BAD_REQUEST, 'Cannot pay for cancelled appointment');
  }
