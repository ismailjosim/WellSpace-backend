 ...
 */
export const parseTimeToMs = (timeString: string, defaultMs: number): number => {
...
export const setAuthCookie = (res: Response, token: AuthToken): void => {
  ...
  if (token.accessToken) {
    res.cookie('accessToken', token.accessToken, getAuthCookieOptions(accessTokenMaxAge));
  }

  if (token.refreshToken) {
    res.cookie('refreshToken', token.refreshToken, getAuthCookieOptions(refreshTokenMaxAge));
  }
};
