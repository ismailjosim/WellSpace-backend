...
const createManyNotificationsIntoDB = async (payloads: NotificationPayload[]) => {
  const result = await Promise.all(payloads.map((payload) => createNotificationIntoDB(payload)));
