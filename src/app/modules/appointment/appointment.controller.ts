...
const getAllAppointments = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await AppointmentService.getAllAppointmentFromDB(options, filters);
...
const getMyAppointment = catchAsync(async (req: Request, res: Response) => {
  ...
  const filters = pick(req.query, appointmentFilterableFields);
  const user = req.user as JwtPayload;
  const result = await AppointmentService.getMyAppointmentFromDB(options, filters, user);
...
const createAppointmentWithPayLater = catchAsync(async (req: Request, res: Response) => {
  const user = req.user as JwtPayload;
  const payload = req.body;
  const result = await AppointmentService.createAppointmentWithPayLater(user, payload);
