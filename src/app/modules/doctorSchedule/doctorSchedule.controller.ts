...
const createDoctorSchedule = catchAsync(async (req: Request, res: Response) => {
  const result = await DoctorScheduleService.createDoctorScheduleIntoDB(req.user, req.body);
  sendResponse(res, {
...
const getAllDoctorSchedules = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await DoctorScheduleService.getAllDoctorSchedulesFromDB(filters, options);
  sendResponse(res, {
    statusCode: StatusCode.CREATED,
    success: true,
...
const getMySchedule = catchAsync(async (req: Request, res: Response) => {
  const filters = pick(req.query, scheduleFilterableFields);
  const options = pick(req.query, ['limit', 'page', 'sortBy', 'orderBy']);
  const result = await DoctorScheduleService.getMyScheduleFromDB(filters, options, req.user);
  sendResponse(res, {
    statusCode: StatusCode.CREATED,
    success: true,
